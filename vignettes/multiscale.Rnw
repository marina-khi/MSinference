...
This vignette is organized as follows. Section \ref{sec:multiple} describes the multiscale procedure for comparing different time trends as in \cite{KhismatullinaVogt2020} and displays the results of analysing the COVID-19 data with the help of our test. In the future, we are planning to add two Sections that will present our mutliscale test for analysing a single time trend as in \cite{KhismatullinaVogt2019} and the results of applying it to the temperature data and introduce the estimator of the long-run variance which is needed for analyzing a nonparametric regression with errors of class AR($p$). %Section \ref{sec:concl} concludes.
